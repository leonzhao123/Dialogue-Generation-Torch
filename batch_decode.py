...
def decode_batch(model, big_batch_data, batch_size,
                 bert_model="unilm_v2_bert_pretrain", max_seq_length=256,
                 num_qkv=0, beam_size=5, max_tgt_length=40):
    tokenizer = BertTokenizer.from_pretrained(bert_model, do_lower_case=False)
    data_tokenizer = WhitespaceTokenizer()
    tokenizer.max_len = max_seq_length
    # pair_num_relation = 0
    ...
    bi_uni_pipeline.append(
        seq2seq_loader.Preprocess4Seq2seqDecoder(
            ...
            max_seq_length, max_tgt_length=max_tgt_length,
            mode="s2s", num_qkv=num_qkv,
            s2s_special_token=False, s2s_add_segment=False,
            s2s_share_segment=False,  pos_shift=False
        )
    )
    ...
    for cur_iter in range(total_iter):
        ...
        if cur_iter % 100 == 0:
            print("{} / {}...".format(cur_iter, total_iter))
