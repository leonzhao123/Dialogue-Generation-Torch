...
recall_idx_file = open("recall_idx.txt", "w", encoding="utf-8")
...
def compute_selection_score(data_path, src_path, top):
    ...
    for query, knows in query_know_dict.items():
        ...
        max_num_index_list = map(bleu_list.index, heapq.nlargest(top, bleu_list))
    print("compute_selection_score", count / len(query_know_dict))
...
def get_rank_know(data_path, src_path, out_path):
    ...
    with open(out_path, "w", encoding="utf-8") as out:
        count = 0
        for idx, (query, knows) in enumerate(query_know_dict.items()):
            check_sent = knows[0].split("\t")[0]
            ...
            if check_sent in line:
                count += 1
                recall_idx_file.write(str(idx) + "\n")

            out.write(line)
            out.write("\n")
        print("get_rank_knowledge_1", len(query_know_dict))
        print("get_rank_knowledge_2", count / len(query_know_dict))
