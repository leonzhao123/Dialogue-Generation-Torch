...
class C_Seq2SeqDataset(torch.utils.data.Dataset):
	...
	def __init__(self, file_src, file_tgt, file_check, file_style, batch_size, tokenizer, max_len, file_oracle=None, short_sampling_prob=0.1,
		...
		if file_oracle is None:
			with open(file_src, "r", encoding='utf-8') as f_src, open(file_tgt, "r", encoding='utf-8') as f_tgt,\
				open(file_check, "r", encoding='utf-8') as f_check, open(file_style, "r", encoding='utf-8') as f_style:

				for src, tgt, check, style in zip(f_src, f_tgt, f_check, f_style):
					src = src.split("[SEP]")
					tgt = tgt.split("[SEP]")
					style = style.split("[SEP]")
					...
					for i in range(TopK):
						...
						style_tk = tokenizer.tokenize(style[i].strip())
						assert len(src_tk) > 0
						assert len(tgt_tk) > 0
...
class Preprocess4Seq2seq_predict(Pipeline):
	""" Pre-processing steps for pretraining transformer """
